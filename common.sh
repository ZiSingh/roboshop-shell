...
mongo_schema_setup() {
  ...
  mongo --host mongodb-dev.erdevops.store <${app_path}/schema/$component.js  &>>$log_file
  stat_check $?
}
...
mysql_schema_setup() {
  ...
  echo -e "${color} Load Schema ${nocolor}"
  mysql -h mysql-dev.erdevops.store -uroot -p${mysql_root_password} </app/schema/${component}.sql   &>>$log_file
  stat_check $?
}
